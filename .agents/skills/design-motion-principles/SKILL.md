...
version: '1.1'
description: Audit UI motion with context-aware restraint, polish, accessibility, performance, and experimentation checks.
---
...
Applies to UI animations, transitions, hover states, layout movement, loading,
error, modal, tab, toast, and other motion behavior.

## When to use

Use when reviewing or designing motion. Do not apply any designer's preference
without first identifying the product context and existing interaction language.

## Required behavior

- Inspect `AGENTS.md`, `package.json`, existing motion primitives, and component structure first.
- Search for both existing animations and motion gaps in conditional UI.
- State the inferred product context, animation style, gaps, and proposed
  Emil/Jakub/Jhey weighting before the full audit; stop for confirmation.
- After confirmation, read `audit-checklist.md`, the weighted designer references,
  mandatory accessibility guidance, and topical references as needed.
- Check reduced motion, interruptibility, enter/exit symmetry, layout movement,
  performance, optical alignment, and whether motion carries meaning.

## Verification

- Report findings with severity, file/line evidence, and a concrete recommendation.
- Include a quick summary, overall assessment, and separate per-designer views.
- Verify `prefers-reduced-motion` and avoid claiming a motion issue without code evidence.

## Exclusions

Do not skip reconnaissance, force a universal style, or proceed from the
initial context proposal to the full audit without user confirmation.
...
1. **AGENTS.md** — Any explicit repository context about the project's purpose or design intent
