...
describe('issue labels attach proxy route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    global.fetch = vi.fn();
  });
  ...
  it('forwards POST requests', async () => {
    ...
    await POST(createMockRequest({ body: { labelId: 3 } }), {
      params: Promise.resolve({ id: '77' })
    });

    expect(global.fetch).toHaveBeenCalledWith(
      'http://localhost:3001/api/v1/tickets/77/labels',
      expect.objectContaining({
        method: 'POST',
        body: JSON.stringify({ labelId: 3 })
      })
    );
  });
});
