import { beforeEach, describe, expect, it, vi } from 'vitest';

import { GET, PATCH } from './route';

import type { NextRequest } from 'next/server';

vi.mock('~/lib/runtime-config', async () => {
  const actual =
    await vi.importActual<typeof import('~/lib/runtime-config')>('~/lib/runtime-config');
  return {
    ...actual,
    getVersionedApiBaseUrl: vi.fn(() => 'http://localhost:3001/api/v1')
  };
});

function createMockRequest(
  headers: Record<string, string | undefined> = {},
  cookies: Record<string, string | undefined> = {}
): NextRequest {
  return {
    headers: {
      get: (name: string) => headers[name.toLowerCase()] ?? null
    },
    cookies: {
      get: (name: string) => {
        const value = cookies[name];
        return value ? { name, value } : undefined;
      }
    }
  } as unknown as NextRequest;
}

describe('GET /api/auth/me route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    global.fetch = vi.fn();
  });

  it('forwards authorization and tenant headers from request', async () => {
    (global.fetch as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: true,
      json: async () => ({ data: { userId: '1' } })
    });

    const req = createMockRequest({
      authorization: 'Bearer header-token',
      'x-tenant-id': '123'
    });

    const res = await GET(req);
    const json = (await res.json()) as { data: { userId: string } };

    expect(json.data.userId).toBe('1');
    expect(global.fetch).toHaveBeenCalledWith(
      'http://localhost:3001/api/v1/iam/identity',
      expect.objectContaining({
        method: 'GET',
        headers: expect.objectContaining({
          authorization: 'Bearer header-token',
          'x-tenant-id': '123'
        })
      })
    );
  });

  it('falls back to cookies when authorization/tenant headers are missing', async () => {
    (global.fetch as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: true,
      json: async () => ({ data: { userId: '1' } })
    });

    const req = createMockRequest(
      {},
      {
        accessToken: 'cookie-token',
        tenantId: '456'
      }
    );

    await GET(req);

    expect(global.fetch).toHaveBeenCalledWith(
      'http://localhost:3001/api/v1/iam/identity',
      expect.objectContaining({
        method: 'GET',
        headers: expect.objectContaining({
          authorization: 'Bearer cookie-token',
          'x-tenant-id': '456'
        })
      })
    );
  });
});

describe('PATCH /api/auth/me route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    global.fetch = vi.fn();
  });

  it('forwards auth and tenant headers and request body', async () => {
    (global.fetch as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: true,
      json: async () => ({ data: { displayName: 'New Name' } })
    });

    const req = {
      ...createMockRequest(
        {
          authorization: 'Bearer header-token',
          'x-tenant-id': '123'
        },
        {}
      ),
      json: vi.fn(async () => ({ displayName: 'New Name' }))
    } as unknown as NextRequest;

    const res = await PATCH(req);
    const json = (await res.json()) as { data: { displayName: string } };

    expect(json.data.displayName).toBe('New Name');
    expect(global.fetch).toHaveBeenCalledWith(
      'http://localhost:3001/api/v1/iam/identity',
      expect.objectContaining({
        method: 'PATCH',
        headers: expect.objectContaining({
          authorization: 'Bearer header-token',
          'x-tenant-id': '123'
        }),
        body: JSON.stringify({ displayName: 'New Name' })
      })
    );
  });

  it('forwards phoneNumber in PATCH body when provided', async () => {
    (global.fetch as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: true,
      json: async () => ({ data: { phoneNumber: '+14155552671' } })
    });

    const req = {
      ...createMockRequest(
        {
          authorization: 'Bearer header-token',
          'x-tenant-id': '123'
        },
        {}
      ),
      json: vi.fn(async () => ({ displayName: 'New Name', phoneNumber: '+14155552671' }))
    } as unknown as NextRequest;

    await PATCH(req);

    expect(global.fetch).toHaveBeenCalledWith(
      'http://localhost:3001/api/v1/iam/identity',
      expect.objectContaining({
        method: 'PATCH',
        body: JSON.stringify({ displayName: 'New Name', phoneNumber: '+14155552671' })
      })
    );
  });
});
