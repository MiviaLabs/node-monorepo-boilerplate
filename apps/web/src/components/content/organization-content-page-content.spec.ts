...
async function readOrganizationContentPageContentSource() {
  const fs = await import('node:fs/promises');
  const filePath = path.join(__dirname, 'organization-content-page-content.tsx');
  return fs.readFile(filePath, 'utf-8');
}

describe('organization content page architecture', () => {
  it('renders server-fed organization pages through the shared content workspace', async () => {
    const source = await readOrganizationContentPageContentSource();
    ...
    expect(source).toContain('router.replace(`/content/${updated.slug}`);');
  });
});
