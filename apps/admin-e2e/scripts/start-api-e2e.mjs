...
if (!connectionFile) {
  ...
  console.log('[admin-e2e] Missing test DB connection file. Starting API test stack...');
...
if (!connectionFile) {
  // eslint-disable-next-line no-console
  console.error(
    `[admin-e2e] Missing test DB connection file after setup. Checked: ${connectionFileCandidates.join(
      ', '
    )}.`
  );
  process.exit(1);
...
if (!databaseUrl) {
  // eslint-disable-next-line no-console
  console.error('[admin-e2e] DATABASE_URL missing in .test-db-connection.json');
  process.exit(1);
}
...
const env = {
  ...
  EVENTS_DATABASE_URL: eventsDatabaseUrl,
...
// eslint-disable-next-line no-console
console.log(
  '[admin-e2e] Starting API server for Playwright flow with testcontainers-backed services'
);
// eslint-disable-next-line no-console
console.log(
  '[admin-e2e] Runtime:',
...
// eslint-disable-next-line no-console
console.log('[admin-e2e] API build completed, launching dist/apps/api/main.js');
...
child.on('exit', (code, signal) => {
  clearInterval(parentWatchdog);
  // eslint-disable-next-line no-console
  console.log('[admin-e2e] API child exited', { code, signal });
  if (signal) {
    process.exit(0);
  }
  process.exit(code ?? 1);
